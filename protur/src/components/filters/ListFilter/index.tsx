...
const contenido = [  {    categoria: 'Ciudades',    elementos: ['lugares/01.jpeg', 'lugares/02.jpeg', 'lugares/03.jpg', 'lugares/04.jpeg']
  },
  {
    categoria: 'Rutas',
...
export const ListFilter = (props: any) => {
  ...
  const [contenidoFiltrado, setContenidoFiltrado] = useState(contenido.filter((item) => item.categoria === categoriaSeleccionada));
    ...
    <div className='listFilter'>
      <div className='grid px-5 md:p-0 grid-cols-2 lg:grid-cols-8 gap-6'>
        {categorias.map((categoria) => (
          <button
            className={`rounded-3xl border-solid border-[#CCCCCC] text-[#999999] bg-transparent ${categoriaSeleccionada === categoria ? 'activo' : ''}`}
            key={categoria}
      ...
      {categoriaSeleccionada && (
      <ul className={`p-5 md:p-0 grid md:grid-cols-2 lg:grid-cols-3 gap-24 my-10 elementos ${animation.showAppearClass ? 'aparecer' : ''} ${animation.transiciones ? 'transition-all duration-500' : ''}`}>
